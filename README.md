# Debian 10 docker image with MiKTeX build environment

## Obtaining the image

Get the latest image from the registry:

    docker pull miktex/miktex-build-debian:buster

or build it yourself:

    docker build --tag miktex/miktex-build-debian:buster .
...
Build the MiKTeX deb package:
    ...
    mkdir -p ~/work/miktex/builds/buster
    ...
    docker run -t \
      ...
      -v ~/work/miktex/builds/buster:/miktex/build:rw \
      -e USER_ID=`id -u` \
      -e GROUP_ID=`id -g` \
      miktex/miktex-build-debian:buster

The build artifact `miktex-*.deb` will be written to
`~/work/miktex/builds/buster`.
